...
class SelectScene2: SKScene{
    ...
    override func touchesBegan(touches: NSSet, withEvent    event: UIEvent) {
        for touch: AnyObject in touches {
            ...
            if menu1.containsPoint(location) && gameInfo?.locked > 3 {
                gameInfo?.difficulty = 3
                gameInfo?.nextScene = 3
                gameInfo?.gameID = 3
                delegate_escape!.sceneEscape(self)
            } else if menu2.containsPoint(location) && gameInfo?.locked > 4 {
                gameInfo?.difficulty = 3
                gameInfo?.nextScene = 3
                gameInfo?.gameID = 4
                delegate_escape!.sceneEscape(self)
            } else if menu3.containsPoint(location) && gameInfo?.locked > 5 {
                gameInfo?.difficulty = 3
                gameInfo?.nextScene = 3
                gameInfo?.gameID = 5
                delegate_escape!.sceneEscape(self)
